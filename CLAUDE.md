...
Zapier CLI integration for [Notifuse](https://www.notifuse.com), a self-hosted and cloud-hosted email marketing platform. **Sending transactional email is what the integration is for** — `Send Transactional Email` is the headline action, and the rest supports it. Triggers are REST Hooks layered on Notifuse's existing outbound webhook subsystem; actions call its RPC API.

**The backend lives in a separate repository** ([Notifuse/notifuse](https://github.com/Notifuse/notifuse), Go + PostgreSQL), and this app requires Notifuse 39.1 or newer — the release that grants a Zapier connection's key `transactional` read and write, without which the send action's notification picker answers 403. The user-facing flow it must stay consistent with is documented at [docs.notifuse.com/integrations/zapier](https://docs.notifuse.com/integrations/zapier). Payload shapes are produced by PL/pgSQL triggers in that repo, so **no compiler in this repository can see them change**. That single fact drives most of the rules below.

Status: scaffolded and green — six REST Hook triggers, three actions, four dropdowns. `npm test`, `npm run typecheck` and `npx zapier-platform validate` all pass; nothing has been pushed or promoted. *Architecture* below describes what exists.
...
src/
  ...
  creates/               sendTransactionalEmail, upsertContact, subscribeToList
  dropdowns/             workspace, transactionalNotification, list, segment,
                         customFields
